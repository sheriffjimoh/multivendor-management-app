...
.table-responsive{
  ...
  <section class="content-header">
        ...
        <li class="active">Paid offer List</li>
      </ol>
    ...
    <section class="content">
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class='alert alert-info alert-dismissible'>
             ...
             Hi, Your paid offer appears  here!
              Thanks.
            </div>
            <div class="box-body">
          ...
          <ul class="list-group">
           ...
           $sql=$conn->query("SELECT  * FROM supply  WHERE  Status='on' and Payment='on' AND Supplier='$userid' order by id desc ");
           if ($sql->rowCount() > 0) {
                     while ($row = $sql->fetch()) {
           $balance = $row['Balance'];
              ...
              $expense = $row['Expenses'];
               $return = $row['Returned'];
                ...
                $returnprice = $price*$row['Returned'];}
             if (!empty($balance) || !empty($expense)) {
               $expandbal = $balance + $expense;
     ...
     <li class="list-group-item">Prod-price<span class="label label-info pull-right"><?php echo $row['Prod_price']; ?></span></li>     
       ...
       <li class="list-group-item">Prod-peices<span class="label label-info pull-right"><?php echo $row['Prod_peices']; ?></span></li>
         <li class="list-group-item">Returned-peices<span class="label label-info pull-right"><?php echo (!empty($row['Returned']))? $row['Returned'] : 0; ?></span></li>
         <li class="list-group-item">Expenses<span class="label label-info pull-right"><?php echo (!empty($row['Expenses']))? number_format( $row['Expenses'],2)  :0;    ?></span></li>
         <li class="list-group-item">Remain-balance<span class="label label-info pull-right"><?php echo (!empty($balance))? number_format($balance,2)  :0; ?></span></li>
             <li class="list-group-item">Total-amount<span class="label label-info pull-right"><?php echo (!empty($row['Total']))? number_format( $row['Total'],2)  :0;   ?></span></li>
       <li class="list-group-item">Amount-payable<span class="label label-info pull-right"><?php echo  (!empty($Totalamount))? number_format($Totalamount , 2)  :0; ?></span></li>  
        ...
        <li class="list-group-item">Payment<span class="label label-success pull-right">Cleared</span></li> 
       <li class="list-group-item">Status<span class="label label-success pull-right">Cleared</span></li> 
      <br>               
                 <?php }     } else{?>
                  <h2 class="text-warning lead">Seems you have no paid order. </h2>
