...
.table-responsive{
 ...
 echo "<br>";  $remain =0;
              $name =$_GET['name'];
              $size = $_GET['size'];
            $sql=$conn->query("SELECT  * FROM production where Prod_name='$name'  and prod_size='$size' ");
           if ($sql->rowCount() > 0) {
                     while ($row = $sql->fetch()) {
                        $remain += $row['Remain'];
 $name = $row['Prod_name']; 
  ...
  $size = $row['prod_size'];
      ...
      <h2 class="text-warning lead">Seems this product as  not been in production,kindly  check out some other! </h2>
       ...
       <?php }  }else{ 
  echo ' <h3 class="text-info">Check for available  product before booking, click  <mark>view</mark> for   Details</h3> ';
