    ...
    <section class="content">
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <ul class="list-group">
           ...
           $sql=$conn->query("SELECT  * FROM supply  WHERE  Payment='off' AND Status='off'  order by id desc ");
           if ($sql->rowCount() > 0) {
                     while ($row = $sql->fetch()) {?>
     ...
     <li class="list-group-item">Prod-price<span class="label label-info pull-right"><?php echo $row['Prod_price']; ?></span></li>     
       ...
       <li class="list-group-item pull-right">
        <a  class='btn btn-warning btn-sm  btn-flat' onclick='return confirm("this attempt is to clear this order,please note that you must verified before clear this  !");' href='../class/process/staff-update.php?clear_supply=<?php echo $id ?>' ><i class='fa fa-check'></i> Clear</a>
</li>  <br>               
                 <?php }     } else{?>
